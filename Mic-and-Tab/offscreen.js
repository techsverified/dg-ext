...
async function startRecording() {
  ...
  return new Promise((resolve, reject) => {
    navigator.mediaDevices.getDisplayMedia({ video: true, audio: true }).then(async screenAudio => {
      ...
      alert("No mic permissions! Please allow microphone in extension");
      reject(new Error("Mic permissions error"));  // reject the promise 
      return; 
      ...
      screenAudio.getAudioTracks().forEach(track => {
          const source = audioContext.createMediaStreamSource(new MediaStream([track]));
          source.connect(merger, 0, 0); // add channel 1
      });
      

      micStream.getAudioTracks().forEach(track => {
          const source = audioContext.createMediaStreamSource(new MediaStream([track]));
          source.connect(merger, 0, 1);  // add channel 2
      });

      const mixedStream = audioContext.createMediaStreamDestination();
      merger.connect(mixedStream); // the concoction

      const recorder = new MediaRecorder(mixedStream.stream, { mimeType: 'audio/webm' });
      // purposely over-riding the mimeType since getDisplayMedia() was merged.
